...
def exec_cmd(cmd, cwd=None, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=False) -> (int, AnyStr, AnyStr):
    try:
        ...
        # print('exec cmd Exception: ' + str(e) + ', ' + str(cmd))
...
def exec_jar(jar_path: str, jar_params: list) -> (int, AnyStr, AnyStr):
    pre_cmd = ['java', '-jar', jar_path]
    pre_cmd.extend(jar_params)
    return exec_cmd(pre_cmd)
