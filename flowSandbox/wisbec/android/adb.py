...
class Adb(object):
    ...
    @classmethod
    def set_device_id(cls, device_id: str):
        cls.m_device_id = device_id
    ...
    @classmethod
    def reboot(cls):
        return Adb.exec('-s', cls.m_device_id, 'reboot')
    ...
    @classmethod
    def set_adb_proxy(cls, host, port):
        try:
            ret = Adb.set_proxy(cls.m_device_id, host, port)
        except Exception as err:
            ret = False
            Log.error('set proxy failed:{}'.format(err))
        return ret

    @classmethod
    def cancel_proxy(cls):
        code, out, err = Adb.shell(cls.m_device_id, 'settings delete global http_proxy')
        code, out, err = Adb.shell(cls.m_device_id, 'settings delete global global_http_proxy_host')
        code, out, err = Adb.shell(cls.m_device_id, 'settings delete global global_http_proxy_port')
        cls.reboot()
        return code == 0
    ...
    def get_app_name(cls, device_id: str, pkg_name: str):
        if not cls.is_path_exists(device_id, '/sdcard/android-tool.apk'):
            ...
            cls.push(device_id, PackageResource.get_mitm_cert_path(), '/sdcard/Download')
        code, out, err = cls.shell(device_id,
                                   'CLASSPATH=/sdcard/android-tool.apk',
                                   'app_process', '/', 'android.tools.Main',
                                   'debug', '--app-name', pkg_name)
        if code != 0:
            raise AdbException('get app name failed:{} {} '.format(err, pkg_name))
        else:
            return out.strip()
    ...
    @classmethod
    def stop_service_s(cls, device_id, *args):
        """
        return true if service is not started or service stopped success
        """
        code, out, err = Adb.shell(device_id, 'am', 'stopservice', *args)
        out = out.strip()
        if 'Service stopped' in (out + err) or 'Service not stopped: was not running' in (out + err):
            return True, None
        else:
            return False, (out + err)
    ...
    @classmethod
    def set_prop(cls, device_id: str, property_name: str, value: str):
        code, out, err = cls.su_shell(device_id, 'setenforce', '0')
        if code != 0 or out + err != '':
            raise AdbException('set prop failed:{}'.format(err))

        code, out, err = cls.shell(device_id, 'setprop', property_name, value)
        if code != 0:
            raise AdbException('set prop failed:{}'.format(err))
        else:
